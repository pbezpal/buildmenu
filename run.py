...
def getSelfConfig():
    ...
    pathlib.Path(script_dir+'/config').mkdir(parents=True,exist_ok=True, mode=0o777)
    t = tempfile.mkdtemp()
...
def getSources(project):
    shell("rm -rf "+src_dir)
    pathlib.Path(src_dir+"/"+project['name']).mkdir(parents=True, exist_ok=True, mode=0o777)
