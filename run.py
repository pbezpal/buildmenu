...
def getSelfConfig():
    ...
    pathlib.Path(script_dir+'/config').mkdir(exist_ok=True, mode=0o755)
    t = tempfile.mkdtemp()
...
def getSources(project):
    shell("rm -rf "+src_dir)
    pathlib.Path(src_dir).mkdir(mode=0o755, exist_ok=True)
    pathlib.Path(src_dir+"/"+project['name']).mkdir(mode=0o755, exist_ok=True)
